...
'use client';
import { useActionState, useEffect, useState, useTransition, ChangeEvent } from 'react';
import { createResource, FormState } from '@/app/actions/admin';
...
export default function ResourceForm({ resource, accessToken }: {resource: string, accessToken: string | null}) {
  ...
  const {s, sN} = useServices(session?.user.id, status);
  ...
  useEffect(() => {
    const loader = async () => {
      ...
      const dataClients = await fetchAllClients(accessToken);
      setUsers(data);
  ...
  return (
    <form action={formAction} className="space-y-4">
      {state.success === false && (
        <p className="text-red-500">{state.error}</p> // ✅ Affiche l'erreur
      )}
      {state.success === true && (
        <p className="text-green-500">Ressource créée avec succès !</p>
      )}
      {resource === 'clients' && (
        <>
          <div>
