...
export const SubscriptionInfo = ({ subscriptions }: { subscriptions: SubscriptionWithService }) => {
  ...
  return (
    <div className="bg-blue-50 p-4 rounded-lg">
        <>
          {subscriptions ? (
            <div key={subscriptions.id} className="space-y-3 p-4 bg-white rounded-lg shadow-sm mb-4">
              <div className="flex justify-between items-start">
                  {subscriptions.status === "active" ? 
                    (<div>
                      ...
                      <div>
                        <p className="font-semibold text-lg">{subscriptions.service.name || 'Inconnu'}</p>
                      </div>
                    )
                  }               
                <span className={`px-3 py-1 rounded-full text-sm ${getStatusStyle(subscriptions.status)}`}>
                  {getStatusLabel(subscriptions.status)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                {subscriptions.start_date && (
                  <div>
                    ...
                    <p className="font-medium">{formatDate(subscriptions.start_date)}</p>
                  </div>
                )}
                {subscriptions.next_payment_date && (
                  <div>
                    <p className="text-gray-500">Prochaine échéance</p>
                    <p className="font-medium">{formatDate(subscriptions.next_payment_date)}</p>
                  </div>
                )}
                {subscriptions.end_date && (
                  <div>
                    <p className="text-gray-500">Fin d'abonnement</p>
                    <p className="font-medium">{formatDate(subscriptions.end_date)}</p>
                    {subscriptions.status === 'active' && (
                      <p className="text-xs text-gray-500 mt-1">
                        {getDaysRemaining(subscriptions.end_date)} jours restants
                      </p>
                    )}
                  </div>
                )}
