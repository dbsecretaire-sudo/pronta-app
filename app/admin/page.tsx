    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {resources.map((resource) => {
          const className = `block bg-white p-6 rounded-lg shadow hover:shadow-md transition-transform transform hover:scale-105 border border-gray-100`;
          const href = `${resource.path}?accessToken=${session?.accessToken}`
          return (
          <Link
            key={resource.path}
            href={href}
